...
switch($accion){
        case"Agregar":
           ...
           $sentenciaSQL= $conexion ->prepare("INSERT INTO productos_cannabis (nombre, imagen, precio) VALUES (:nombre, :imagen, :precio);");
           ...
           header("Location:productos cannabis.php");
           break;

            case"Modificar":
                $sentenciaSQL= $conexion->prepare("UPDATE productos_cannabis SET nombre=:nombre WHERE id=:id");
                $sentenciaSQL->bindParam(':nombre', $txtnameofproduct);
                $sentenciaSQL->bindParam(':id', $txtID);
                $sentenciaSQL->execute();
                
                if($txtimagen!=""){
                    $sentenciaSQL= $conexion->prepare("UPDATE productos_cannabis SET imagen=:imagen WHERE id=:id");
                ...
                $sentenciaSQL= $conexion->prepare("UPDATE productos_cannabis SET precio=:precio WHERE id=:id");
                $sentenciaSQL->bindParam(':precio', $txtprice);
                $sentenciaSQL->bindParam(':id', $txtID);
                $sentenciaSQL->execute(); 
                header("Location:productos cannabis.php");              
            ...
            case "Seleccionar":
                $sentenciaSQL= $conexion->prepare("SELECT * FROM productos_cannabis WHERE id=:id");
                $sentenciaSQL->bindParam(':id', $txtID);
                $sentenciaSQL->execute();
            ...
            case "Borrar":
                ...
                $sentenciaSQL= $conexion->prepare("SELECT imagen FROM productos_cannabis WHERE id=:id");
                $sentenciaSQL->bindParam(':id', $txtID);
                $sentenciaSQL->execute();
                $Productos=$sentenciaSQL->fetch(PDO::FETCH_LAZY); 
                ...
                $sentenciaSQL= $conexion->prepare("DELETE FROM productos_cannabis WHERE id=:id");
                $sentenciaSQL->bindParam(':id', $txtID);
                $sentenciaSQL->execute();
                header("Location:productos cannabis.php");
                break; 
...
$sentenciaSQL= $conexion->prepare("SELECT * FROM productos_cannabis ");
$sentenciaSQL->execute();
